...
let { upper, middle = remaining.fourth, lower } = doorCode;
console.log(`Sequence: ${upper}, ${middle}, ${lower}`);
...
function revealPath({ direction, distance }) {
	console.log(`${direction}, ${distance}ft`);
}
revealPath({ direction: "185*", distance: 200 });
...
mixPotion({ ingredient2: "Lemon Juice" });
...
function castSpell([spellOne, spellTwo, ...spells]) {
	console.log(`Cast ${spellOne}, then ${spellTwo}`);
}
castSpell(["Abra", "Cadabra", "Bipity", "Boppity", "Boop"]);
